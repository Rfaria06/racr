...
export class TracksCreatePage{
  ...
  constructor(private fb: FormBuilder, private api: TrackApiService, private router: Router) {
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      this.api.create(new New_Track("", this.api.apiService.getAuthStore().model!["id"], formValue.name, formValue.description, formValue.image, formValue.URL, formValue.created, formValue.updated));
